...
interface UseWebSocketConfig {
  ...
  onRtpCapabilities: (rtpCapabilities: any) => void
  onTransportCreated: (data: any) => void
  onProduced: (data: any) => void
  onConsumerCreated: (data: any) => void
  onProducer: (data: any) => void
      ...
      ws.onmessage = async (e) => {
        try {
          ...
          if (data.type === "rtpCapabilities") {
            config.onRtpCapabilities(data.data)
          }

          if (data.type === "transportCreated") {
            config.onTransportCreated(data.data)
          }

          if (data.type === "produced") {
            config.onProduced(data.data)
          }

          if (data.type === "consumerCreated") {
            config.onConsumerCreated(data.data)
          }

          if (data.type === "producer") {
            config.onProducer(data.data)
          }

          if (data.type === "producerclosed") {
            config.onProducerClosed()
          }

          if (data.type === "consumerclosed") {
            config.onConsumerClosed()
          }
